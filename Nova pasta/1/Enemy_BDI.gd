...
onready var ataque = preload("res://scene/enemy_Neptanas_ataque.tscn")
...
func _fixed_process(delta):
	...
	#ai
	var Move = Vector2()
	var body = get_node("Area_Enemy_Neptanas").get_overlapping_bodies()
...
func _on_Body_area_enter( area ):
	if area.get_name() == "player_ataque":
		...
		life = life - dmg
		if (life <= 0):
			get_node("/root/global_enemy").set_enemy_stat("Enemy_Neptanas","dead")
